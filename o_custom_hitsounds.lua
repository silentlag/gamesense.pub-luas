...
local client_set_event_callback,
...
local add_to_searchpath =
    bind_signature(
    "filesystem_stdio.dll",
    "VFileSystem017",
    "\x55\x8B\xEC\x81\xEC\xCC\xCC\xCC\xCC\x8B\x55\x08\x53\x56\x57",
...
local create_directory =
    ...
    "filesystem_stdio.dll",
    "VFileSystem017",
    "\x55\x8B\xEC\x81\xEC\x08\x01\x00\x00\x53",
    "void(__thiscall*)(void*, const char*, const char*)"
)

local sndplaydelay = cvar.sndplaydelay
local native_Surface_PlaySound =
    vmt_bind("vguimatsurface.dll", "VGUI_Surface031", 82, "void(__thiscall*)(void*, const char*)")
...
local function init()
    ...
    local root_path = ffi.string(current_path)
    local full_path = string.format("%s\\csgo\\sound\\hitsounds", root_path)

    create_directory("hitsounds", "XGAME")
    add_to_searchpath(full_path, "XGAME", 0)

    local function refresh_sound_list()
        sound_names = {}
        sound_name_to_file = {}
        ...
        if #sound_names == 0 then
            init_sound("No sounds found", nil)
        end
    end
    ...
    ui.new_button(
        ...
        function()
            ...
            client.color_log(
                ...
                200,
                0,
                "[HitSound] \x01Found \x04" .. #sound_names .. "\x01 sound(s). Reload the script to update the list."
            )
        end
    )
    ...
    ui.new_button(
        "LUA",
        "B",
        "Copy hitsound folder path",
        function()
            local clipboard = require("gamesense/clipboard")
            clipboard.set(full_path)
            client.color_log(52, 191, 91, "[HitSound] \x01Path copied: \x04" .. full_path)
        end
    )
